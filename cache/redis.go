...
// 获取redis连接池对象
func GetRedisPool(address, password string, database int) *redis.Pool {
	pool := &redis.Pool{
		MaxIdle:     50,
		...
		Dial: func() (redis.Conn, error) {
			return dial("tcp", address, password, database)
		},
	}
	return pool
}

func dial(network, address, password string, database int) (redis.Conn, error) {
...
func RedisFlushDb(pool *redis.Pool) (string, error) {
	conn := pool.Get()
	defer conn.Close()
	...
	s, err := redis.String(conn.Do("flushdb"))
	if err != nil {
		return "", err
	}
	return s, nil
