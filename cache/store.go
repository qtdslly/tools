...
import (
	...
	"sync"
	"time"
	"ymminer/internal/common/logger"
...
func NewStore(redisAddr, redisPassword string, database int, redisTTL, memTTL int, lazy bool) *Store {
	s := new(Store)
	s.pool = GetRedisPool(redisAddr, redisPassword, database)
